...
where
    ...
    D: Dna,
{
    pub fn reproduce(&mut self, scores: &[FiniteF64], rate: NormalizedF64) {
...
/// impl: softmax
pub fn probabilities(scores: &[FiniteF64]) -> impl Iterator<Item = NormalizedF64> + Clone + '_ {
    let scores = scores.iter().map(|x| f64::exp(x.get()));
    let sum = scores.clone().sum::<f64>();
    scores
